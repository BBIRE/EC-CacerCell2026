#!/usr/bin/env Rscript
###############################################################################
## Differential Co-expression Network (DCN) analysis (igraph-based)
## - Single, de-duplicated, production-style script
## - Centralized file directory: network/network_06/corr06
## - Robust to igraph 2.x membership typing and messy edge weights
## - Generates:
##    * per-network: module ranking, topology, GO:BP enrichment, module plots,
##      optional circlize module-chord plot, optional module meta-graph
##    * pairwise: global topology dashboard, ECDFs, scatterplots, module Jaccard,
##      edge overlap plot
##
## NOTES:
## - This script expects igraph objects (.rds) with:
##      * vertex names in V(g)$name (gene symbols)
##      * edge weights in E(g)$weight (correlation or delta correlation)
## - If weights are missing or non-numeric, they are sanitized to numeric.
###############################################################################
...
###############################################################################
## 3) Compare two networks (single canonical function)
###############################################################################

compare_networks_plots <- function(g1, g2,
                                  label1 = "Net1",
                                  label2 = "Net2",
                                  outdir = file.path(OUT_COMPARE, "NET1_vs_NET2")) {
  stopifnot(inherits(g1, "igraph"), inherits(g2, "igraph"))
  dir.create(outdir, showWarnings = FALSE, recursive = TRUE)

  # Sanitize weights (prevents Louvain / betweenness failures on non-numeric weights)
  g1 <- sanitize_graph_weights(g1, default = 0)
  g2 <- sanitize_graph_weights(g2, default = 0)

  # ---- global metrics table (one-row per network) ----
  global_metrics <- function(g) {
    ...
    deg <- igraph::degree(g)
    g_lcc <- safe_lcc(g)
    apl <- tryCatch(igraph::average.path.length(g_lcc, directed = FALSE),
  ...
  # ---- node metrics for ECDF/scatter ----
  node_metrics <- function(g) {
    if (igraph::vcount(g) == 0) {
      return(data.frame(gene = character(), degree = integer(), strength = numeric(), btw = numeric(),
                        stringsAsFactors = FALSE))
    }

    gene <- igraph::V(g)$name %||% as.character(seq_len(igraph::vcount(g)))
    wabs <- abs(igraph::E(g)$weight %||% 1)
    wlen <- 1 / pmax(1e-9, wabs)

    data.frame(
      gene = gene,
      degree = igraph::degree(g),
      strength = igraph::strength(g, weights = wabs),
      btw = igraph::betweenness(g, directed = FALSE, weights = wlen, normalized = FALSE),
      stringsAsFactors = FALSE
    )
  }

  gm1 <- global_metrics(g1); gm1$Network <- label1
  gm2 <- global_metrics(g2); gm2$Network <- label2
  gmtab <- rbind(gm1, gm2)

  # ---- 1) global dashboard plot ----
  ...
  if (nrow(gml) > 0) {
    p_global <- ggplot2::ggplot(gml, ggplot2::aes(x = variable, y = value, fill = Network)) +
      ggplot2::geom_col(position = ggplot2::position_dodge(width = 0.7), width = 0.6) +
      ggplot2::coord_flip() +
      ggplot2::labs(x = NULL, y = NULL, title = "Global topology comparison") +
      ggplot2::theme_minimal(base_size = 12)
    save_png(file.path(outdir, "01_global_topology.png"), p_global, width = 10, height = 6, dpi = 200)
  }
  ...
  plot_ecdf <- function(col, ttl) {
    ggplot2::ggplot(ecdf_long, ggplot2::aes(x = log1p(.data[[col]]), color = Network)) +
      ggplot2::stat_ecdf(geom = "step", linewidth = 1) +
      ggplot2::labs(x = paste0("log1p(", ttl, ")"), y = "ECDF", title = paste0("ECDF of ", ttl)) +
      ggplot2::theme_minimal(base_size = 12)
  }

  save_png(file.path(outdir, "02_ecdf_degree.png"),      plot_ecdf("degree", "degree"),       6, 4, 200)
  save_png(file.path(outdir, "03_ecdf_strength.png"),    plot_ecdf("strength", "strength"),   6, 4, 200)
  save_png(file.path(outdir, "04_ecdf_betweenness.png"), plot_ecdf("btw", "betweenness"),     6, 4, 200)

  # ---- 3) node-wise scatter (only if overlap of genes exists) ----
  nodes_merged <- merge(nm1, nm2, by = "gene", all = FALSE, suffixes = c(".1", ".2"))

  if (nrow(nodes_merged) > 0) {
    scat <- function(xcol, ycol, ttl) {
      ggplot2::ggplot(nodes_merged, ggplot2::aes(x = log1p(.data[[xcol]]), y = log1p(.data[[ycol]]))) +
        ggplot2::geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "gray60") +
        ggplot2::geom_point(alpha = 0.5, size = 1.2) +
        ggplot2::labs(
          x = paste0(label1, " log1p(", ttl, ")"),
          y = paste0(label2, " log1p(", ttl, ")"),
          title = paste0(ttl, ": ", label1, " vs ", label2)
        ) +
        ggplot2::theme_minimal(base_size = 12)
    }
    save_png(file.path(outdir, "05_scatter_degree.png"),   scat("degree.1", "degree.2", "Degree"),   6, 5, 200)
    save_png(file.path(outdir, "06_scatter_strength.png"), scat("strength.1", "strength.2", "Strength"), 6, 5, 200)
  }

  # ---- 4) module overlap (Jaccard) ----
  jaccard_modules <- function(gA, gB) {
    wA <- abs(igraph::E(gA)$weight %||% 1)
    wB <- abs(igraph::E(gB)$weight %||% 1)

    clA <- igraph::cluster_louvain(gA, weights = wA)
    clB <- igraph::cluster_louvain(gB, weights = wB)
    ...
    J <- matrix(0, nrow = length(A), ncol = length(B),
                dimnames = list(paste0("A", A), paste0("B", B)))
  ...
  if (igraph::vcount(g1) > 0 && igraph::vcount(g2) > 0 && igraph::ecount(g1) > 0 && igraph::ecount(g2) > 0) {
    ...
    pheatmap::pheatmap(
      ...
      filename = file.path(outdir, "06_module_overlap_jaccard.png"),
      width = 8, height = 6
    )
  }
  ...
  edge_df <- data.frame(
    Class = c(paste0("Overlap (", label1, "∩", label2, ")"),
              paste0("Only ", label1),
              paste0("Only ", label2)),
    Count = c(length(intersect(e1, e2)),
              length(setdiff(e1, e2)),
              length(setdiff(e2, e1))),
    stringsAsFactors = FALSE
  )

  p_edges <- ggplot2::ggplot(edge_df, ggplot2::aes(x = Class, y = Count, fill = Class)) +
    ggplot2::geom_col(width = 0.6) +
    ggplot2::guides(fill = "none") +
    ggplot2::labs(x = NULL, y = "Edges", title = "Edge overlap / rewiring") +
    ggplot2::theme_minimal(base_size = 12)

  save_png(file.path(outdir, "08_edge_overlap.png"), p_edges, 7, 4, 200)

  invisible(list(global = gmtab, nodes_merged = nodes_merged))
}

###############################################################################
## 4) Analyze a single network (single canonical function)
###############################################################################
...
###############################################################################
## 5) Example runner (edit file names to match what you have in FILEDIR)
###############################################################################
## If you already have graphs saved elsewhere, change these paths accordingly.
## The key point is: all paths are built from FILEDIR.

# Example expected inputs (adjust filenames to your actual ones)
f_g1  <- file.path(FILEDIR, "DCN_graphs_08c4a5f8534f4497_EC_group1.rds")
f_g2  <- file.path(FILEDIR, "DCN_graphs_08c4a5f8534f4497_Healthy_group2.rds")
f_g3  <- file.path(FILEDIR, "DCN_graphs_98b61808d296fb76_Decidua_group1.rds")

# Load only if files exist (prevents hard failures in partial runs)
read_graph_if_exists <- function(path) {
...
gEC  <- read_graph_if_exists(f_g1)
gDec <- read_graph_if_exists(f_g2)
gH   <- read_graph_if_exists(f_g3)

if (!is.null(gEC))  res_EC  <- analyze_one_graph(gEC,  label = "EC",      outdir = OUT_ANALYSIS)
if (!is.null(gDec)) res_Dec <- analyze_one_graph(gDec, label = "Decidua", outdir = OUT_ANALYSIS)
if (!is.null(gH))   res_H   <- analyze_one_graph(gH,   label = "Healthy", outdir = OUT_ANALYSIS)

if (!is.null(gEC) && !is.null(gDec)) {
  compare_networks_plots(gEC, gDec, label1 = "EC", label2 = "Decidua",
                         outdir = file.path(OUT_COMPARE, "EC_vs_Decidua"))
}
if (!is.null(gEC) && !is.null(gH)) {
  compare_networks_plots(gEC, gH, label1 = "EC", label2 = "Healthy",
                         outdir = file.path(OUT_COMPARE, "EC_vs_Healthy"))
}
if (!is.null(gDec) && !is.null(gH)) {
  compare_networks_plots(gDec, gH, label1 = "Decidua", label2 = "Healthy",
                         outdir = file.path(OUT_COMPARE, "Decidua_vs_Healthy"))
}

###############################################################################
## 6) Differential DCN analysis — READS EXISTING GRAPHS (RDS)
## Outputs:
